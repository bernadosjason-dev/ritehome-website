...
**No real-time notification exists here** — a submission only ever lands as a spreadsheet row; nothing pings anyone. The form's on-page confirmation deliberately does not state a response-time commitment because of this (unlike Hannah's Telegram-backed one above). If that's ever added, wire an actual notification into `Code.gs` first (e.g. the same Telegram bot Hannah uses) — don't just add the sentence.
