...
- **Kitchen sizing must sum every wall/run the customer gives, never just one**: a real bug — a customer describing an L-shape kitchen as "4 meters by 3 meters" got quoted off the 4m number alone, ignoring the 3m wall — showed the model was matching PRICING's small/medium/large rows against a single number instead of the shape's total run length. `FACTS` now has a `KITCHEN LAYOUT SHAPES` section mapping the real layout names (Straight, L-shape, U-shape, Galley/Parallel, Island — the same shapes the internal estimator uses) to Small/Medium/Large by typical total run length, and explicitly instructs the model to add multiple given measurements together before matching a row, and to flag rather than under-quote a total that clearly exceeds the Large bracket (~6m+). `RESPONSE_FORMAT` offers these shape names as chips for Kitchen specifically, since customers know their kitchen's shape before they know its total length.
